...
Reader::Reader(string files, int sC, int arg): inputFile(files), numFiles(0), selectedClass(sC), compareArg(arg){};
...
int Reader::selectedClassExists(){
    for (unsigned int i = 0; i < fileList.size(); i++) {
        int temp = extractClass(fileList.at(i));
        if (selectedClass == temp){
            return 0;
        }
    }
    cerr << "The class you selected does not exist in the files given." << endl;
    return -1;
}
...
void Reader::perceptronStart(){
    perceptron.resize(65);
    double y;
    double b;
    int d;
    double tempTotal;
    for(int e = 0; e < 100; e++){ //100 epochs
        for (int i = 0; i < numFiles; i++){ //each image
            //d is 1 if data comes from arg Class, -1 if not
            b = perceptron.at(64);
            d = setD(selectedClass,i);
            tempTotal = 0;
            for(int n = 0; n < 64; n++){ //each node inside the histograms
                //calculate sum(0-63 of w[i] * h[i])
                tempTotal += perceptron.at(n) * vCluster.at(i).cNormAt(n);
            }
            y = b + tempTotal;
            for(int w = 0; w < 64; w++){ //each node inside the perceptron
                //calculate y, y = b + sum(0-63 of w[i] * h[i])
                perceptron.at(w) = perceptron.at(w) + (d - y)*vCluster.at(i).cNormAt(w); 
            }
        
            //update bias, b = b + (d-y)
            //cout << "B:" << b << endl;
            perceptron.at(64) = b + (d - y);
...
int Reader::setD(int a, int i) {
    if (extractClass(fileList[i]) == a){
        return 1;
    }
    else return -1;
}
