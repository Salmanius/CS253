...
void Cluster::hStart(){
    switch (arg3) {
        ...
        case 4: { //need to set all the averages for 9histo
            for (int i = 0; i < 9; i++){
                for (int j = 0; j < 64; j++){
                    clusterNorm9Average[i][j] = images[0].normHistoNine[i][j];
                }
            }
            break;
        }
...
void Cluster::makeAverage(){
    ...
    switch (arg3) {
        ...
        case 4: { //averaging the 9 histograms
            double sum = 0.0f;
            int size = images.size();
            if (size > 1){
                 for (int h = 0; h < 9; h++){ 
                    for (int i = 0; i < 64; i++) {
                        sum = 0.0f;
                        for (int j = 0; j < size; j++) {
                            sum += images[j].normHistoNine[h][i];
                        }
                        sum = sum/size;
                        clusterNorm9Average[h][i] = sum;
                    }
                 }
            }
...
int Cluster::findMax(){
    ...
    for (int i = 0; i < size; i++){
        temp = 0;
        for (int j = 0; j < size; j++){
            if (images[i].getClassName() == images[j].getClassName()){
                temp++;
            }
        }
        if (temp > max){
            maxC = temp;
            max = images[i].getClassName();
        }
    }
    return maxC;
